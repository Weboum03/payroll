...
const filterData = async(filterValues) => {
    logs.value = await getBatchLogsByType(route.params.id, 'release', filterValues)
}
...
onMounted( async () => {
    ...
    logs.value = await getBatchLogsByType(route.params.id, 'release');
