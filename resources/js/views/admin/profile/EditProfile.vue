...
watch(immediateManager, async (current, previous) => {
    ...
    immediateManagerCode.value = current;
})
...
watch(leaveApprovingAuth, async (current, previous) => {
    ...
    leaveApprovingAuthCode.value = current;
})
...
const schemas = [
    yup.object({
        ...
        .test('is-exists', 'This Email is already taken.', async (value) => {
            ...
            let checkResponse = await checkDuplicacy('email', {value: value, user_id: route.params.id  });
        ...
        .test('is-exists', 'This Phone is already taken.', async (value) => {
            if (value == null || value == '') {
                return true;
            }
            let checkResponse = await checkDuplicacy('phone', {value: value, user_id: route.params.id  });
            if(checkResponse.data == false) { return true }
            return false;
        }),
